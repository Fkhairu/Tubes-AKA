...
void menuPencarian() {
    ...
    string namaCari;
    cout << "\nMasukkan NAMA yang ingin dicari (Case Sensitive/Huruf Besar Kecil harus sama): ";
...
void jalankanAnalisis() {
    ...
    for (int n : ukuranInput) {
        ...
        binarySearchRecursive(data, target, 0, n - 1);
        auto stopRec = high_resolution_clock::now();

        cout << left << setw(15) << n 
             << setw(20) << duration_cast<nanoseconds>(stopIter - startIter).count() 
             << setw(20) << duration_cast<nanoseconds>(stopRec - startRec).count() 
             << "Selesai" << endl;
    }
}